...
export default function Dashboard() {
    ...
    const stats = props?.stats || { utilisateurs: 0, clients: 0, comptes: 0, solde_total: 0 };
    const adminThemeColor = '#1e40af';

    const usersHref = typeof route !== 'undefined' ? route('admin.users.index') : '/admin/users';
    const comptesHref = typeof route !== 'undefined' ? route('gestionnaire.comptes.index') : '/gestionnaire/comptes';
    const transactionsHref = typeof route !== 'undefined' ? route('caissier.transactions.index') : '/caissier/transactions';
        ...
        <AdminLayout>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '16px', marginTop: '20px' }}>
                <div style={{ background: '#fff', padding: '16px', borderRadius: '8px' }}>
                    ...
                    <div style={{ fontSize: '20px', fontWeight: '700' }}>{stats.utilisateurs}</div>
                </div>
                <div style={{ background: '#fff', padding: '16px', borderRadius: '8px' }}>
                ...
                <div style={{ background: '#fff', padding: '16px', borderRadius: '8px' }}>
                    ...
                    <div style={{ fontSize: '20px', fontWeight: '700' }}>{formatCurrency(stats.solde_total)}</div>
                </div>
            </div>

            <div style={{ marginTop: '28px', display: 'flex', gap: '12px' }}>
                <Link href={usersHref} className="db-action-btn" style={{ padding: '10px 14px', background: adminThemeColor, color: '#fff', borderRadius: '8px', textDecoration: 'none', fontWeight: '600' }}>
                    Gérer les utilisateurs
                </Link>

                <Link href={comptesHref} className="db-action-btn" style={{ padding: '10px 14px', background: '#0d9488', color: '#fff', borderRadius: '8px', textDecoration: 'none', fontWeight: '600' }}>
                    Aller aux comptes (Gestionnaire)
                </Link>

                <Link href={transactionsHref} className="db-action-btn" style={{ padding: '10px 14px', background: '#2563eb', color: '#fff', borderRadius: '8px', textDecoration: 'none', fontWeight: '600' }}>
